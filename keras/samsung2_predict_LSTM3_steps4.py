...
sam_x1 = sam_x[:, : ,:5]
sam_x2 = sam_x[:, : ,[5,6,9,10,11,13]]
# print(sam_x1.shape, sam_x2.shape) # (1162, 4, 6) (1162, 4, 6)


# 아모레 5일치 x 와 다음날의 시가 y
amore_x, amore_y = split_xy5(amore, 4, 1)
...
amore_x1 = amore_x[:,:,:5] 
amore_x2 = amore_x[:,:,[5,6,9,10,11,13]]
# print(amore_x1.shape, amore_x2.shape)   # (1162, 4, 6) (1162, 4, 6)

# predict_value
sam1_pred = sam[-4:,:5].reshape(1,sam_x1.shape[1] * sam_x1.shape[2])
sam2_pred = sam[-4:,[5,6,9,10,11,13]].reshape(1,sam_x2.shape[1] * sam_x2.shape[2])
amore1_pred = amore[-4:,:5].reshape(1, amore_x1.shape[1] * amore_x1.shape[2])
amore2_pred = amore[-4:,[5,6,9,10,11,13]].reshape(1, amore_x2.shape[1] * amore_x2.shape[2])
...
# print(sam1_train.shape, sam1_test.shape)    # (871, 24) (291, 24)
# print(sam2_train.shape, sam2_test.shape)    # (871, 24) (291, 24)
...
print(amore1_train.shape, amore1_test.shape)    # (871, 24) (291, 24)
print(amore2_train.shape, amore2_test.shape)    # (871, 24) (291, 24)
...
shape11 = int(sam1_x_train.shape[1])
shape12 = int(sam1_x_train.shape[2])
shape21 = int(sam2_x_train.shape[1]*2)
shape22 = int(sam2_x_train.shape[2]/2)




# 삼전1
sam1_train = np.reshape(sam1_train, (sam1_x_train.shape[0], shape11, shape12))
sam1_test = np.reshape(sam1_test, (sam1_x_test.shape[0], shape11, shape12))
# 삼전2
sam2_train = np.reshape(sam2_train, (sam2_x_train.shape[0], shape21, shape22))
sam2_test = np.reshape(sam2_test, (sam2_x_test.shape[0], shape21, shape22))
# 아모레1
amore1_train = np.reshape(amore1_train, (amore1_x_train.shape[0], shape11, shape12))
amore1_test = np.reshape(amore1_test, (amore1_x_test.shape[0], shape11, shape12))
# 아모레2
amore2_train = np.reshape(amore2_train, (amore2_x_train.shape[0], shape21, shape22))
amore2_test = np.reshape(amore2_test, (amore2_x_test.shape[0], shape21, shape22))

# 2-1. 삼전 모델1
input1 = Input(shape=(shape11, shape12))
...
# 2-2. 삼전 모델2
input2 = Input(shape=(shape21, shape22))
...
# 2-3. 아모레 모델 1
input3 = Input(shape=(shape11, shape12))
...
# 2-4. 아모레 모델 2
input4 = Input(shape=(shape21, shape22))
...
merge4 = Dense(512, activation='relu', name='mg3')(merge3)   
...
model.fit([sam1_train, sam2_train, amore1_train, amore2_train], sam_y_train,
          epochs =400, batch_size=32, verbose=1, validation_split=0.2,
          callbacks=[es, mcp])
...
# predict
# reshape
sam1_pred = sam1_pred.reshape(1,shape11, shape12)
sam2_pred = sam2_pred.reshape(1,shape21, shape22)
amore1_pred = amore1_pred.reshape(1,shape11, shape12)
amore2_pred = amore2_pred.reshape(1,shape21, shape22)
