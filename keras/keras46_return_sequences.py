...
# 2. 모델구성
model = Sequential()                    # input : (None, 3, 1)
model.add(LSTM(64, activation='relu', input_shape=(3, 1), 
               return_sequences=True))      # -> output : (N, 64)
#return_sequences default = false
model.add(LSTM(32, activation='relu'))      
# 첫번째 LSTM 의 output shape 는 2차원이기 때문에, return_sequences 없이 그냥 실행하면 dimension error가 발생한다.
# LSTM의 expect input_dim은 3차원
